...
def _install_fake_nfsim(monkeypatch):
    ...
    monkeypatch.setattr(bngsim_model._runtime, 'bngsim', fake_pkg)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_AVAILABLE', True)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True)
    return calls
...
def _install_fake_nf_sessions(monkeypatch, *, has_nfsim=True, has_rulemonkey=True):
    ...
    monkeypatch.setattr(bngsim_model._runtime, 'bngsim', fake_pkg)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_AVAILABLE', True)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', has_nfsim)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_RULEMONKEY', has_rulemonkey)
    return calls
...
def test_normalize_nf_action_method_rejects_rulemonkey_when_unavailable(monkeypatch, method):
    monkeypatch.setattr(bngsim_model._runtime.bngsim,
        'normalize_method',
...
def test_initialize_models_uses_bngsim_when_available(monkeypatch, tmp_path):
    ...
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_network_generation):
        with patch.object(algorithms.base, 'BngsimModel', FakeBngsimModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
...
def test_initialize_models_bionetgen_backend_skips_bngsim(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_network_generation):
        with patch.object(algorithms.base, 'BngsimModel', UnexpectedBngsimModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_nf_bionetgen_backend_skips_bngsim(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=AssertionError('should not generate XML')):
        with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
            with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True):
                models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_no_bngsim_env_disables_nf_auto(monkeypatch, tmp_path):
    """NF-only BNGL with auto backend + PYBNF_NO_BNGSIM=1 must fall back to
    the BNGLModel subprocess path (no XML generation, no bngsim NF bridge)."""
    model = _make_tfun_bngl_model(tmp_path, method='nf')
    output_dir = tmp_path / 'pybnf_output'
    output_dir.mkdir()
    algo = _make_dummy_algorithm(model, output_dir)
    ...
    assert len(models) == 1
    assert isinstance(models[0], pset.BNGLModel)
    # Must not have been promoted to a bngsim NF bridge.
    assert not isinstance(models[0], bngsim_model.BngsimNfModel)


def test_initialize_models_no_bngsim_env_disables_hybrid_auto(monkeypatch, tmp_path):
    """Hybrid (generate_network + NF simulate) BNGL with auto backend +
    PYBNF_NO_BNGSIM=1 must fall back to NetModel (BNG2.pl already produced
    the .net during the generates_network branch; bngsim NF must not run)."""
    ...
    assert len(models) == 1
    assert isinstance(models[0], pset.NetModel)
    # bngsim NF bridge must not have been used for hybrid either.
    assert not isinstance(models[0], bngsim_model.BngsimNfModel)
...
def test_initialize_models_bngsim_backend_rejects_unavailable_bngsim(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=AssertionError('should fail before BNG2.pl')):
        with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', False):
            with patch.object(algorithms.base, 'BNGSIM_ERROR', 'bngsim is not available'):
                with pytest.raises(printing.PybnfError, match='bngsim is not available'):
                    algorithms.Algorithm._initialize_models(algo)


def test_initialize_models_bngsim_backend_rejects_when_pybnf_no_bngsim(monkeypatch, tmp_path):
    """Explicit `bngl_backend = bngsim` with PYBNF_NO_BNGSIM=1 must surface an
    error that names the env var, not the generic 'bngsim is not available'.

    BNGSIM_AVAILABLE is captured at import time; the realistic path is for
    _bngsim_runtime_available() to return False because the env var is set
    while BNGSIM_AVAILABLE is still True.
    """
    model = _make_tfun_bngl_model(tmp_path)
    output_dir = tmp_path / 'pybnf_output'
    output_dir.mkdir()
    ...
    with patch.object(algorithms.base, 'run_subprocess', side_effect=AssertionError('should fail before BNG2.pl')):
        with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
            with pytest.raises(printing.PybnfError, match='PYBNF_NO_BNGSIM is set'):
                algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_bngsim_backend_rejects_missing_nfsim(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=AssertionError('should fail before XML')):
        with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
            with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', False):
...
def test_initialize_models_falls_back_to_netmodel_when_bridge_init_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_network_generation):
        with patch.object(algorithms.base, 'BngsimModel', BrokenBngsimModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_uses_bngsim_nf_when_supported(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_xml_generation):
        with patch.object(algorithms.base, 'BngsimNfModel', FakeBngsimNfModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True):
                    models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_uses_bngsim_rulemonkey_when_supported(monkeypatch, tmp_path):
    ...
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_xml_generation):
        with patch.object(algorithms.base, 'BngsimNfModel', FakeBngsimNfModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', False):
                    with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_RULEMONKEY', True):
                        models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_nf_falls_back_when_bridge_init_fails(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_xml_generation):
        with patch.object(algorithms.base, 'BngsimNfModel', BrokenBngsimNfModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True):
                    models = algorithms.Algorithm._initialize_models(algo)
...
def test_classify_actions_for_bngsim_returns_hybrid_with_rulemonkey_alias(monkeypatch):
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_RULEMONKEY', True)
    assert bngsim_model.classify_actions_for_bngsim([
        'generate_network({overwrite=>1})',
...
def test_initialize_models_hybrid_uses_bngsim_nf(monkeypatch, tmp_path):
    ...
    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_fake_hybrid_generation):
        with patch.object(algorithms.base, 'BngsimNfModel', FakeBngsimNfModel):
            with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
                with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True):
                    models = algorithms.Algorithm._initialize_models(algo)
...
def test_initialize_models_hybrid_explicit_bngsim_backend_errors_on_xml_failure(monkeypatch, tmp_path):
    ...
    def _net_ok_then_xml_fail(cmd, timeout, stdout=None, stderr=None, input=None):
        del timeout, stdout, stderr, input
        bngl_path = Path(os.getcwd()) / cmd[1]
        bngl_text = bngl_path.read_text()
        if 'writeXML' in bngl_text:
            raise RuntimeError('forced hybrid XML failure')
        _write_dummy_net(bngl_path.with_suffix('.net'))

    monkeypatch.chdir(tmp_path)
    with patch.object(algorithms.base, 'run_subprocess', side_effect=_net_ok_then_xml_fail):
        with patch.object(algorithms.base, 'BNGSIM_AVAILABLE', True):
            with patch.object(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM', True):
                with pytest.raises(printing.PybnfError, match='hybrid XML generation failed'):
                    algorithms.Algorithm._initialize_models(algo)
...
def _install_fake_session_raising_timeout(monkeypatch, *, backend, timeout, elapsed):
    ...
    monkeypatch.setattr(bngsim_model._runtime, 'bngsim', fake_bngsim)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_AVAILABLE', True)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_NFSIM',
                        backend == bngsim_model.BNGSIM_NF_BACKEND_NFSIM)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_HAS_RULEMONKEY',
...
def _install_fake_simulator_raising(monkeypatch, exc):
    ...
    monkeypatch.setattr(bngsim_model._runtime, 'bngsim', fake_bngsim)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_AVAILABLE', True)
    monkeypatch.setattr(bngsim_model._runtime, 'BNGSIM_VERSION', '9.9.9-test')
    return fake_bngsim
