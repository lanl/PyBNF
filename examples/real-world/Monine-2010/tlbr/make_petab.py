#!/usr/bin/env python
"""Generate a PEtab v2 bundle for the Monine-2010 tlbr job.

The runnable ``tlbr.conf`` is the single source of truth. Its PEtab.v2 compliance
...
any change to ``tlbr.conf`` / ``tlbr.bngl`` / ``tlbr.exp`` so the bundle can't drift
from the conf. For Monine-2010/tlbr the export emits 1 observable
(``observableFormula`` = the bare ``FL`` function name), 3 estimated parameters and 12
measurements; because the job perturbs (a 12-dose ``LTconc`` scan) it also emits the
conditions and experiments tables -- 12 conditions over 12 experiments, one per dose.
The ``.exp`` carries no ``_SD`` column, so the noise is a constant unit sigma and the
objective is plain sum-of-squares.
