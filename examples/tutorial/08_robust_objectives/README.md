...
**Feature:** the `noise_model` surface — Gaussian vs Laplace (outlier robustness) · **Difficulty:** ★★☆
...
Laplace fit must recover `k` to within 3%, and the Gaussian fit must be *provably*
wrong — off by at least 10% — so the comparison is never vacuous.
...
heavy-tailed noise model — `laplace` — and let the fit decide which points to
trust.
