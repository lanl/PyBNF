...
**Profiled Noise Scale**:
An *estimated* **Noise Parameter Source** (a `fit <parameter>` scale) whose value is not searched but **solved for** at every evaluation, from the closed-form maximum-likelihood expression over the scored points that share it — the weighted residual RMS for the Gaussian families, the weighted mean absolute residual for Laplace. Opt-in per run (`noise_profiling = 1`, ADR-0108, #562). The group is keyed by the free parameter's own name, so one σ shared across observables is one group and per-observable σ names are one group each. It removes search dimensions (32 parameters in 13 of the 23 Grein subset-I slugs), makes *every* draw a global sampler ranks scale-optimal instead of ranking draws by how wrong their sampled σ happens to be, and leaves no box for σ to run into. The parameter stays *declared* (its bounds and prior go inert) and stays *estimated*, so it keeps counting in `k` for AIC/BIC; it is not a coordinate of the best **PSet**, so its value is reported in `Results/profiled_noise.txt` rather than `sorted_params_*.txt`. Distinct from the **Analytic Per-Series Scale** (ADR-0066), which profiles an unknown *multiplicative scale on the prediction* by a criterion that is not the objective's own minimizer — which is why that one has a nonzero `∂c*/∂θ` (ADR-0099) and this one, by the envelope theorem, contributes no gradient term at all. Distinct also from *marginalizing* a nuisance, which is what a posterior does — hence the refusal for the Bayesian samplers.
_Avoid_: sigma elimination, hierarchical sigma, marginalized noise (it is maximized out, not integrated out)
...
A restatement of the fit as a *larger* problem — extra internal variables plus equality constraints that tie them back together — that coincides with the original at the solution but can be far better conditioned on the way there. **Multiple Shooting** is the first consumer; direct collocation, latent-state estimation, and path constraints are the same shape. Implemented as a reusable layer (`pybnf/transcription`, ADR-0109, #563) rather than as a `job_type`: the transcription is the part that differs between those methods, and everything above it — the **Augmented Variable Layout**, the equality residual/Jacobian interface, the **Augmented-Lagrangian Outer Loop**, the **Transcription Homotopy**, and **Certified Reconstruction** — is shared. The layer contains no dynamics, no simulator call, and no configuration key; a consumer implements `objective_at` / `equality_at` / `certify` against it. Motivated by a landscape finding, not a search finding: on `Borghans_BiophysChem1997` only a −4.5 %/+2.3 % window in *period* beats a flat line, so under single shooting the no-dynamics solution is the ceiling almost everywhere in the box and fifteen independent global searches reach it.
_Avoid_: reformulation, relaxation, problem transformation

**Augmented Variable Layout**:
The flat vector `[ u_reported | z_1 | … | z_K ]` a **Constrained Transcription** searches: the fit's own free parameters in their leading contiguous slice, then the transcription's internal auxiliary blocks. An auxiliary variable is searched, bounded, and differentiated exactly like a **Free Parameter** and is **never** a reported fit result — it has no scientific meaning and reporting it would claim the fit estimated quantities it did not. `AugmentedLayout` enforces that structurally: internal names are namespace-qualified (`seg2::A_state`) with a separator no parameter name can contain, a collision refuses at construction, and `reported_of(u)` is the single accessor every reporting/certification/**PSet** path uses. Every coordinate is already in the space the optimizer walks (**Parameter Scale** sampling space for the reported block), so the layout transforms nothing. ADR-0109.
_Avoid_: extended parameter vector, decision vector (unqualified), state vector

**Continuity Defect**:
The equality residual a **Constrained Transcription** imposes — for **Multiple Shooting**, `c_j = Phi_j(z_j, θ) − z_{j+1}`, how far segment *j*'s endpoint is from segment *j+1*'s declared start. Always reported and tested **scaled** (`c_i / s_i`, typically by the state's own magnitude): a defect is a difference of *states*, so a model spanning six orders of magnitude in species would otherwise hand the penalty term a condition number for free, and scaling is an exact reparameterisation the multiplier absorbs. Held with its Jacobian as a **block-sparse** structure (contiguous constraint-group × variable-block regions, additively accumulated) with a future *condensing* seam — eliminating the auxiliary variables to recover a system of the fit's own dimension. ADR-0109.
_Avoid_: gap, mismatch, shooting residual

**Augmented-Lagrangian Outer Loop**:
The driver of a **Constrained Transcription**: it owns the multipliers `λ` and the penalty `ρ`, and hands an *inner solver* the plain bound-constrained smooth problem `L_A = f + λᵀc + ρ/2‖c‖²`. Optimizer-agnostic by contract — the inner solver is any callable reading a box and a per-point model, offered in all three forms PyBNF's optimizers consume (scalar for `lbfgs`, a stacked least-squares residual for `trf` — exact, by completing the square, up to a reported constant `‖λ‖²/(2ρ)` — and Gauss-Newton for `gntr`, the MVP's choice). Classical Hestenes-Powell updates in the Conn-Gould-Toint frame, with three departures: the schedule starts **tight** (`ρ₀ = 10`, `γ = 5`, measured to beat a loose start on both quality and cost), convergence is a **measured** projected-gradient KKT test rather than the inner solver's own flag, and a point already feasible to tolerance never raises `ρ`. The constraint terms never enter `f` — an estimated **Noise Parameter** fitted to residuals that included continuity violation would absorb it as measurement noise and the reported objective would stop being comparable. ADR-0109.
_Avoid_: penalty method (that is the `λ = 0` special case), outer iteration loop

**Transcription Homotopy**:
A ladder of **Constrained Transcription**s of one fit, coarsening toward the ordinary problem — for **Multiple Shooting**, `4-2-1` segments. Not a refinement to add later: the coarsening *is* the mechanism (every segmented stage of the motivating solve scored worse than a flat line; the trace `m=8: -132.32 → m=4: -166.52 → m=2: -221.91 → m=1: -248.07` is the whole result), and it starts in the *middle* rather than at the many-short-segments end, which measured worse than its own start under partial observability. Auxiliary variables carry between stages **by name**; multipliers do not (a coarsened constraint set makes an old multiplier an estimate of nothing). Two rules that disagree on purpose: continue from the last point, report the best **Certified Reconstruction**. ADR-0109.
_Avoid_: continuation (ambiguous with the ODE sense), multigrid, refinement schedule

**Certified Reconstruction**:
The step that keeps a **Constrained Transcription**'s score honest: take an iterate's reported parameters, re-simulate them through the fit's **ordinary unsegmented path**, and score them there. That number — not the augmented objective, which is computed on trajectories that do not join up — is the only one comparable with an ordinary fit's, and the only one a run may report. Performed on **every** outer iterate and the best kept, because a run's final point is not reliably its best one. A transcription that cannot certify produces an explicitly *uncertified* verdict that travels into the run summary, legitimate only for the one-segment rung where the transcription already *is* the single-shoot problem. ADR-0109.
_Avoid_: validation, re-scoring, polish (that is **Refine**)
