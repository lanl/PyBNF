...
picks up the family's moment correction in the transformed space (delegated to the
scale's ``mean_offset``). ``MODE`` is not modeled until a use exercises it.
"""


class LocationInterpretation:
    """Maps a prediction to a Gaussian's additive-space location parameter, as the
    offset to subtract from ``scale.forward(prediction)``."""

    def offset(self, scale, noise):
...
class _Mean(LocationInterpretation):
    def offset(self, scale, noise):
        return scale.mean_offset(noise)
