...
def import_job(problem_yaml_path, out_dir, job_type='de', method='ode',
    ...
    conditions/experiments) is recovered exactly; the *run-recipe* (``job_type``, ``method``,
    ``settings``) is supplied by the caller (see the module docstring). Returns the
    ``out_dir`` path.
    ...
    **row-varying** per-measurement ``observableParameters``/``noiseParameters`` placeholder;
    replicate rows; a fixed SBML parameter an initial assignment, event assignment or algebraic
    rule also sets, or a fixed BNGL parameter the model's actions set) and ``PybnfError``
    for a malformed problem (an ``observableFormula`` symbol that is not a model entity, an
    ambiguous dose-response group, or a fixed row with no nominalValue or naming a model entity
    PEtab does not allow in the parameters table).
    """
