...
class SbmlModel(Model):
    ...
    def execute(self, folder, filename, timeout):
        ...
        for action in self.actions:
            ...
            for t in tasks:
                if t.get('name') == 'Time-Course':
                    time_task = t
                    break
            if not time_task: raise RuntimeError('Time-Course task unexpectedly missing from cps file')
            # Update attributes of time course task
            time_task.set('scheduled', 'true')
            for param in time_task.find_all('cps:Problem/cps:Paramter', namespaces=space):
            ...
            # Todo: check namespace nonsense here
            time_task.append(ET.Element('Report', reference='BNF_Report_1', target='timecourse_output', append='1',
                                        confirmOverwrite='1'))

            # Create the report object
            # Todo: make sure the separator is reproduced literally in the xml file
            report_list = root.findall('cps:ListOfReports', namespaces=space)
            report = ET.Element('Report', key='Report_17', name='TimeCourse', taskType='timeCourse', separator='&#x09;',
                                precision='6')
            report_list.append(report)
            comment = ET.Element('Comment')
            comment.text = 'Automatically generated by BioNetFit'
            report.append(comment)
            report_table = ET.Element('Table', printTitle='1')
            ...
            for s in self.species:
                obj = ET.Element('Object', cn=tablestring % (self.name, s))
                report_table.append(obj)

        # Write the updated Copasi back to file
        cps.write('%s.cps' % file, 'unicode')
        ...
        # Todo: modify the Data parser to work with COPASI output:
